...
def create_feature_report(face_data: Dict[str, List[Dict[str, Any]]]) -> None:
    ...
    report_lines.append("=" * 150)
    report_lines.append("SECTION 4: SUMMARY STATISTICS FOR ALL FEATURES")
    report_lines.append("=" * 150)
    report_lines.append("")
    ...
    for feature_name in feature_names:
        all_values = []
        for person_id in person_ids:
            all_values.extend(person_features[person_id][feature_name])

        if all_values:
            stats_data.append({
                'Feature': feature_name.replace('_', ' ').title(),
                'Min': f"{min(all_values):.4f}",
                'Max': f"{max(all_values):.4f}",
                'Mean': f"{sum(all_values) / len(all_values):.4f}",
                'Count': str(len(all_values))
            })

    headers_stats = ['Feature', 'Min', 'Max', 'Mean', 'Count']
    report_lines.append(format_table(stats_data, headers_stats))
    report_lines.append("")

    # Section 5: Key Insights
    report_lines.append("=" * 150)
    report_lines.append("SECTION 5: KEY INSIGHTS & OBSERVATIONS")
    report_lines.append("=" * 150)
    report_lines.append("")
    ...
    female_values = []
    for person_id in male_ids:
        male_values.extend(person_features[person_id]['aggressive_ratio'])
    for person_id in female_ids:
        female_values.extend(person_features[person_id]['aggressive_ratio'])

    male_avg_agg = sum(male_values) / len(male_values) if male_values else 0
    female_avg_agg = sum(female_values) / len(female_values) if female_values else 0
    agg_delta_pct = ((female_avg_agg - male_avg_agg) / male_avg_agg * 100) if male_avg_agg else 0

    report_lines.append("1. Aggressive Ratio by Gender:")
    report_lines.append(f"   - Males average: {male_avg_agg:.4f}")
    report_lines.append(f"   - Females average: {female_avg_agg:.4f}")
    report_lines.append(f"   - Females show {agg_delta_pct:.2f}% higher aggressive ratio")
    report_lines.append("")

    male_lip_values = []
    female_lip_values = []
    for person_id in male_ids:
        male_lip_values.extend(person_features[person_id]['lip_size_ratio'])
    for person_id in female_ids:
        female_lip_values.extend(person_features[person_id]['lip_size_ratio'])

    male_avg_lip = sum(male_lip_values) / len(male_lip_values) if male_lip_values else 0
    female_avg_lip = sum(female_lip_values) / len(female_lip_values) if female_lip_values else 0
    lip_delta_pct = ((female_avg_lip - male_avg_lip) / male_avg_lip * 100) if male_avg_lip else 0

    report_lines.append("2. Lip Size Ratio by Gender:")
    report_lines.append(f"   - Males average: {male_avg_lip:.4f}")
    report_lines.append(f"   - Females average: {female_avg_lip:.4f}")
    report_lines.append(f"   - Females show {lip_delta_pct:.2f}% higher lip size ratio")
    report_lines.append("")
